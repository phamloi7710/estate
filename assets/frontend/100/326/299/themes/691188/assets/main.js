...
function lib_menuItemList(){
	...
	if (menu_item > menu_limit){
		...
		$('.category-nav > ul').each(function(){
			...
			$(this).append('<li class="more"><a><label>Xem thĂªm ... </label></a></li>');
		});
		$('.category-nav > ul').on('click','.more', function(){
			if($(this).hasClass('less')){
				$(this).html('<a><label>Xem thĂªm ...</label></a>').removeClass('less');
			} else {
	...
	$.ajax({
		...
		success: function(data) {
			...
			setTimeout(function(){
				...
				/* Gá»ŒI Láº I ACTION SAU KHI LOAD */
...
$(document).on('click', '.register_click', function(e) {
	...
	$('.modal_contact___service_info .price-box .special-price').html('GiĂ¡ : ' + price);
